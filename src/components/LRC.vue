       ...
       */
       ...
       var lyrics = res.data.lrcContent.split("\n");
        ...
        for(var i = 0 ;i<lyrics.length;i++){
          ...
          var clause = lyric.replace(timeReg,'');//把前面的歌词的时间部分 替换成空数组 拿到歌词的正文
          console.log(clause)
             for(var k = 0,h = timeRegExpArr.length;k < h;k++) {
                 var t = timeRegExpArr[k];
                 var min = Number(String(t.match(/\[\d*/i)).slice(1)),
                     sec = Number(String(t.match(/\:\d*/i)).slice(1));
                     console.log(String(t.match(/\[\d*/i)).slice(1))
                 // console.log(min+'---'+sec)
                 var time = min * 60 + sec;
                    console.log(time)
                 lrcObj[time] = clause;
    ...
    srcollLrc(key,index){
      const lrcDiv = this.$refs.lrc
       if(key < this.currentTime && key > this.currentTime - (this.keyArr[index+1] - this.keyArr[index])){
           ...
           console.log('key:'+key+'ct:'+this.currentTime+'arr.index+1:'+this.keyArr[index+1]+'arr.index:'+this.keyArr[index])
           console.log( this.currentTime - (this.keyArr[index+1] - this.keyArr[index]))
       }
